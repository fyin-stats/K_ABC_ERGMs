...
prior_sd = 10
imp_rounds <- 2
#samp_size <- rep(NA, imp_rounds)
#samp_scale_fact <- rep(NA, imp_rounds)
...
for(i in 1:M){
 start_time <- Sys.time()
 kbergm_list[[i]] <- kbergm(form=target_mod$formula,prior.mean= 0, prior.sd=prior_sd, samp.size=samp_size, samp.df = samp_df, 
                            samp.scale.fact = samp_scale_fact , est.type = est_type, dist.type = dist_type,transf=transf, 
                            bw.fun = get(bw_fun),kernel.type=kernel_type, kernel.fun = get(kernel_fun) ,bw.type = bw_type, 
...
############################################################################################
saveRDS(kbergm_list,
        "karate_kbergm_list_4442_8000_24000.rds")