...
###################################
#load data
#lazega_network <- intergraph::asNetwork(lazega)
karate_network <- intergraph::asNetwork(karate)
#teenage_network_list <- readRDS("/home/fyin/ABC_for_ERGMs/teenage_network_list.rds")
#cosponsorNetwork <- readRDS("/home/fyin/ABC_for_ERGMs/cosponsorNetwork.rds")
####################################
...
for(j in 1:length(mc_cores)){
 ...
 for(i in 1:M){
  start_time <- Sys.time()
  kbergm_list[[j]][[i]] <- kbergm(target_mod$formula,prior.mean= 0, prior.sd=prior_sd, samp.size=samp_size, samp.df = samp_df, samp.scale.fact = samp_scale_fact , est.type = est_type, dist.type = dist_type,transf=transf, bw.fun = get(bw_fun),kernel.type=kernel_type, kernel.fun = get(kernel_fun) ,bw.type = bw_type, percentage = percentage,
...
karate_time <- cbind(mpletime, samptime, runtime, qtime, mc_cores)
colnames(karate_time) <- c("MPLE", "Sample time", "Total run time", "Quantile finding time", "Number of cores")
saveRDS(karate_time, "karate_time.rds")
#